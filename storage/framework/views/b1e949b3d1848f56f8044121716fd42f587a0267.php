...
<script>
  ...
  function opnpivotmybilling(title,type){
    $.ajax({
        type: 'GET',
        cache: false,
        url: '<?php echo e(url("/viewpivotmybilling")); ?>',
    ...
    $('#tabeltransaction').dxDataGrid({
      dataSource: {
        store: {
          type: 'odata',
          cache: false,
          url: "<?php echo e(url('')); ?>/gettransaction?type=m&stamp=" + new Date().getTime(),
          key: 'notrans',
          beforeSend(request) {
        ...
        if(e.format == 'xlxs'){
          ...
          DevExpress.excelExporter.exportDataGrid({
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
              saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'sisbilling<?php echo e(Carbon\Carbon::now(config("app.GMT"))->format("Y-m-d H:mm")); ?>.xlsx');
            });
          });
          e.cancel = true;
          ...
          DevExpress.pdfExporter.exportDataGrid({
            ...
            doc.save('sisbilling<?php echo e(Carbon\Carbon::now(config("app.GMT"))->format("Y-m-d H:mm")); ?>.pdf');
          });
        }
      },
      scrolling: {
    ...
    $('#tabeltransactionCus').dxDataGrid({
      dataSource: {
        store: {
          type: 'odata',
          cache: false,
          url: "<?php echo e(url('')); ?>/gettransaction?type=c&stamp=" + new Date().getTime(),
          key: 'notrans',
          beforeSend(request) {
            request.params.SaleNo = $('#formsaleh-primary').val();
          },
...
<?php /**PATH C:\xampp\htdocs\sisbilling9\resources\views/getView/viewHome.blade.php ENDPATH**/ ?>